...
from person_wrong_direction import PersonWrongDirection
...
class PlayVideo:
    ...
    def __init__(self, source, cam_name, q):
        self.yolo = None
        self.cap_line = None
        ...
        self.source = source
        self.window_name = cam_name
        self.image_save_path = os.path.join(root, "output/{}".format(cam_name))
        self.q_img = q
        self.line = False
        self.line_coord = None
        self.detections = 'Forward'
        ...
        try:
            ...
            while True:
                ...
                if not self.line:
                    ...
                    self.line = True
                    self.yolo = PersonWrongDirection(self.model, self.line_coord, self.image_save_path)
                ...
                obj = pool.apply_async(self.yolo.predict, (self.q_img, self.detections, self.image))
                frame = obj.get()
...
if __name__ == "__main__":
    ...
    for i in urls:
        url = i['url']
        cam_name = i["name"]
        queue_name.name = cam_name
        queue_list.append(queue_name)
        td = threading.Thread(
            target=PlayVideo(url, cam_name, queue_name).vdo_cap)
        td.start()
        threads.append(td)
